...
merging.** Reading, grepping and `git status` are free; anything that takes a seat
is not. The seat is assigned per session, it cannot be worked out from the repo,
and guessing it wrong is expensive in a way that shows up late: two agents both
believing they hold `main`, or a worker branching in the integrator's checkout.
...
list someone has to extend. The rules left over are these, and they hold whichever
seat you are in — the seat files do not repeat them, except where a seat changes
something about one.
...
version of the rule rather than working around it**, and say what happened
...
that stays in one agent's head is the one thing these conventions cannot
survive.
...
are serialised across agents — see "Shared ground".
